...
TENANT_ID="${WHISKEY_TENANT_ID:-00000000-0000-4000-8000-000000000001}"
WORKER_SUBJECT="${WHISKEY_WORKER_SUBJECT:-whiskey-pilot-worker}"
...
CODEX_CONTROLLER_TOKEN="${WHISKEY_CODEX_APP_SERVER_CONTROLLER_TOKEN:-}"
TENANT_CONTROLLER_PORT="${WHISKEY_TENANT_ROUTING_CONTROLLER_PORT:-18790}"
TENANT_CONTROLLER_TOKEN="${WHISKEY_TENANT_ROUTING_CONTROLLER_TOKEN:-}"
WORKER_CONTROLLER_PORT="${WHISKEY_WORKER_LOAD_CONTROLLER_PORT:-18791}"
WORKER_CONTROLLER_TOKEN="${WHISKEY_WORKER_LOAD_CONTROLLER_TOKEN:-}"
MCP_CONTROLLER_PORT="${WHISKEY_MCP_CONTROLLER_PORT:-18792}"
MCP_CONTROLLER_TOKEN="${WHISKEY_MCP_CONTROLLER_TOKEN:-}"
...
EVAL_RELEASE_CONTROLLER_TOKEN="${WHISKEY_EVAL_RELEASE_CONTROLLER_TOKEN:-}"
...
OBSERVABILITY_CONTROLLER_TOKEN="${WHISKEY_OBSERVABILITY_CONTROLLER_TOKEN:-}"
...
PROVIDER_CONTROLLER_TOKEN="${WHISKEY_PROVIDER_CONTROLLER_TOKEN:-}"
...
APPROVAL_NOTIFICATION_CONTROLLER_TOKEN="${WHISKEY_APPROVAL_NOTIFICATION_CONTROLLER_TOKEN:-}"
...
VAULT_KMS_CONTROLLER_TOKEN="${WHISKEY_VAULT_KMS_CONTROLLER_TOKEN:-}"
VAULT_KMS_VAULT_TOKEN="${WHISKEY_VAULT_TOKEN:-}"
...
FINANCE_CLOSE_CONTROLLER_TOKEN="${WHISKEY_FINANCE_CLOSE_CONTROLLER_TOKEN:-}"
FINANCE_RECONCILIATION_CONTROLLER_TOKEN="${WHISKEY_FINANCE_RECONCILIATION_CONTROLLER_TOKEN:-}"
SCHEDULER_TOKEN="${WHISKEY_SCHEDULER_TOKEN:-${MANDOFORGE_SCHEDULER_TOKEN:-}}"
...
secret_values=()
require_unique_secret() {
  local name="$1"
  local value="$2"
  local existing
  if [[ ${#value} -lt 24 || ! "$value" =~ ^[A-Za-z0-9._+/=-]+$ ]]; then
    echo "$name must be at least 24 characters and use only token-safe characters" >&2
    exit 1
  fi
  case "$value" in
    whiskey-*|replace-*|example-*|changeme*)
      echo "$name must not use a public placeholder value" >&2
      exit 1
      ;;
  esac
  for existing in "${secret_values[@]}"; do
    if [[ "$value" == "$existing" ]]; then
      echo "$name must use a credential distinct from every other Whiskey control-plane token" >&2
      exit 1
    fi
  done
  secret_values+=("$value")
...
require_unique_secret WHISKEY_CODEX_APP_SERVER_CONTROLLER_TOKEN "$CODEX_CONTROLLER_TOKEN"
require_unique_secret WHISKEY_TENANT_ROUTING_CONTROLLER_TOKEN "$TENANT_CONTROLLER_TOKEN"
require_unique_secret WHISKEY_WORKER_LOAD_CONTROLLER_TOKEN "$WORKER_CONTROLLER_TOKEN"
require_unique_secret WHISKEY_MCP_CONTROLLER_TOKEN "$MCP_CONTROLLER_TOKEN"
require_unique_secret WHISKEY_EVAL_RELEASE_CONTROLLER_TOKEN "$EVAL_RELEASE_CONTROLLER_TOKEN"
require_unique_secret WHISKEY_OBSERVABILITY_CONTROLLER_TOKEN "$OBSERVABILITY_CONTROLLER_TOKEN"
require_unique_secret WHISKEY_PROVIDER_CONTROLLER_TOKEN "$PROVIDER_CONTROLLER_TOKEN"
require_unique_secret WHISKEY_APPROVAL_NOTIFICATION_CONTROLLER_TOKEN "$APPROVAL_NOTIFICATION_CONTROLLER_TOKEN"
require_unique_secret WHISKEY_VAULT_KMS_CONTROLLER_TOKEN "$VAULT_KMS_CONTROLLER_TOKEN"
require_unique_secret WHISKEY_VAULT_TOKEN "$VAULT_KMS_VAULT_TOKEN"
require_unique_secret WHISKEY_FINANCE_CLOSE_CONTROLLER_TOKEN "$FINANCE_CLOSE_CONTROLLER_TOKEN"
require_unique_secret WHISKEY_FINANCE_RECONCILIATION_CONTROLLER_TOKEN "$FINANCE_RECONCILIATION_CONTROLLER_TOKEN"
require_unique_secret WHISKEY_SCHEDULER_TOKEN "$SCHEDULER_TOKEN"
...
if [[ ! -f '$REMOTE_ENV' ]]; then cat > '$REMOTE_ENV' <<'ENV'
...
set_env MANDOFORGE_SCHEDULER_TOKEN $SCHEDULER_TOKEN
ensure_env MANDOFORGE_TENANT_ID $TENANT_ID
ensure_env MANDOFORGE_WORKER_SUBJECT $WORKER_SUBJECT
...
set_env MANDOFORGE_CODEX_APP_SERVER_DEPLOYMENT_CONTROLLER_TOKEN $CODEX_CONTROLLER_TOKEN
...
set_env MANDOFORGE_CODEX_APP_SERVER_OPS_CONTROLLER_TOKEN $CODEX_CONTROLLER_TOKEN
...
set_env MANDOFORGE_VAULT_TOKEN $VAULT_KMS_VAULT_TOKEN
...
set_env MANDOFORGE_KMS_TOKEN $VAULT_KMS_CONTROLLER_TOKEN
...
set_env MANDOFORGE_KMS_RECOVERY_CONTROLLER_TOKEN $VAULT_KMS_CONTROLLER_TOKEN
set_env MANDOFORGE_TENANT_ROUTING_MODE single_runtime_tenant
...
set_env MANDOFORGE_TENANT_ROUTING_CONTROLLER_TOKEN $TENANT_CONTROLLER_TOKEN
...
set_env MANDOFORGE_WORKER_LOAD_VALIDATION_CONTROLLER_TOKEN $WORKER_CONTROLLER_TOKEN
...
set_env MANDOFORGE_MCP_DEPLOYMENT_CONTROLLER_TOKEN $MCP_CONTROLLER_TOKEN
...
set_env MANDOFORGE_MCP_ROLLOUT_CONTROLLER_TOKEN $MCP_CONTROLLER_TOKEN
...
set_env MANDOFORGE_MCP_ROLLOUT_ROLLBACK_CONTROLLER_TOKEN $MCP_CONTROLLER_TOKEN
...
set_env MANDOFORGE_AGENT_RELEASE_CONTROLLER_TOKEN $EVAL_RELEASE_CONTROLLER_TOKEN
...
set_env MANDOFORGE_AGENT_RELEASE_DEPLOYMENT_CONTROLLER_TOKEN $EVAL_RELEASE_CONTROLLER_TOKEN
...
set_env MANDOFORGE_AGENT_RELEASE_ORCHESTRATION_CONTROLLER_TOKEN $EVAL_RELEASE_CONTROLLER_TOKEN
...
set_env MANDOFORGE_AGENT_RELEASE_ROLLBACK_CONTROLLER_TOKEN $EVAL_RELEASE_CONTROLLER_TOKEN
...
set_env MANDOFORGE_OBSERVABILITY_COLLECTOR_DEPLOYMENT_CONTROLLER_TOKEN $OBSERVABILITY_CONTROLLER_TOKEN
...
set_env MANDOFORGE_OBSERVABILITY_COLLECTOR_CLUSTER_CONTROLLER_TOKEN $OBSERVABILITY_CONTROLLER_TOKEN
...
set_env MANDOFORGE_OBSERVABILITY_REMEDIATION_CONTROLLER_TOKEN $OBSERVABILITY_CONTROLLER_TOKEN
...
set_env MANDOFORGE_PROVIDER_DEPLOYMENT_CONTROLLER_TOKEN $PROVIDER_CONTROLLER_TOKEN
...
set_env MANDOFORGE_PROVIDER_ROLLOUT_TOKEN $PROVIDER_CONTROLLER_TOKEN
...
set_env MANDOFORGE_PROVIDER_ROLLOUT_ROLLBACK_TOKEN $PROVIDER_CONTROLLER_TOKEN
unset_env DEEPSEEK_API_KEY
unset_env MANDOFORGE_PROVIDER_BASE_URL
unset_env MANDOFORGE_PROVIDER_API_KEY
unset_env MANDOFORGE_PROVIDER_MODEL
...
set_env MANDOFORGE_APPROVAL_NOTIFICATION_DEPLOYMENT_CONTROLLER_TOKEN $APPROVAL_NOTIFICATION_CONTROLLER_TOKEN
...
set_env MANDOFORGE_APPROVAL_NOTIFICATION_OPS_CONTROLLER_TOKEN $APPROVAL_NOTIFICATION_CONTROLLER_TOKEN
...
set_env MANDOFORGE_FINANCE_CLOSE_CONTROLLER_TOKEN $FINANCE_CLOSE_CONTROLLER_TOKEN
...
set_env MANDOFORGE_FINANCE_RECONCILIATION_CONTROLLER_TOKEN $FINANCE_RECONCILIATION_CONTROLLER_TOKEN
chmod 0600 '$REMOTE_ENV'"
...
remote_cmd="cd '$REMOTE_ROOT' && source '$REMOTE_ENV_LOADER' && load_env_file '$REMOTE_ENV' && require_distinct_runtime_tokens"
