...
(() => {
  ...
  const result = {
    ...
    hasCodexAppServer: text.includes('Codex App Server') && text.includes('Check Codex Health') && text.includes('Create Codex Thread') && text.includes('Create Codex Turn') && text.includes('Execute Codex Command') && text.includes('Interrupt Codex Turn') && text.includes('Codex steering'),
