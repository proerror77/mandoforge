...
(() => {
  ...
  const result = {
    ...
    hasVaultReadiness: Boolean(document.querySelector('#vault-readiness')) && text.includes('Secret provider:') && text.includes('STALE ROTATIONS') && text.includes('endpoint missing'),
