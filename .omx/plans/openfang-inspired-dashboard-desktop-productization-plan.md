...
- Phase 1 Semantic view extraction implemented: the semantic memory and ontology surface now lives under `web-ui/src/views/semantic.rs`, with context rendering and graph widgets kept local to that view module.
- Phase 1 Packs view extraction implemented: the WorkflowPack operations surface now lives under `web-ui/src/views/packs.rs`, while shared pack card helpers remain centralized for the next components/helpers cleanup slice.
