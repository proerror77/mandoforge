...
- Phase 5 boundary: browser-native and desktop OS notification forwarding remain intentionally unimplemented until desktop MVP work maps the same notification bridge into Tauri.
...
- Phase 7 hardening truth surface implemented: desktop IPC now exposes `get_desktop_hardening_status` with explicit false values for signed distribution, updater, single-instance, autostart, CSP, native notifications, and enterprise completion claims.
- Phase 7 boundary: the hardening surface is a truthful contract and verification hook, not implementation of signed packaging, updater, autostart, or native notification forwarding.
