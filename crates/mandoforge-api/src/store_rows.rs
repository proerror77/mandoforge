...
pub(crate) fn session_loop_job_from_row(row: PgRow) -> Result<SessionLoopJob, AppError> {
    ...
    Ok(SessionLoopJob {
        ...
        processed_event_seq: row.try_get("processed_event_seq")?,
...
pub(crate) fn task_grant_from_row(row: PgRow) -> Result<TaskGrant, AppError> {
    Ok(TaskGrant {
        ...
        cost_usd_micros_used: row.try_get("cost_usd_micros_used")?,
        semantic_scopes: row.try_get("semantic_scopes")?,
...
pub(crate) fn work_item_from_row(row: PgRow) -> Result<WorkItem, AppError> {
    Ok(WorkItem {
        id: row.try_get("id")?,
        organization_id: row.try_get("organization_id")?,
        team_id: row.try_get("team_id")?,
