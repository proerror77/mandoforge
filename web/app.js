...
function renderCodexAppServer() {
  ...
  const controlDashboard = controlSummary
    ? `<h4>Control-plane Summary</h4>
      ...
      <div class="muted">Ops controller: required ${productionOps.controller_required ? "yes" : "no"} · configured ${productionOps.controller_configured ? "yes" : "no"} · latest ${escapeHtml(productionOps.latest_controller_status || "none")} · validated ${productionOps.latest_controller_validated ? "yes" : "no"} · fresh ${productionOps.controller_evidence_fresh ? "yes" : "no"} · age ${productionOps.latest_controller_age_hours ?? "n/a"}h</div>
      ...
      financeOperations
        ? `<div class="detail-panel">
            ...
            <dl>
              ...
              <dd>required ${financeProductionClose.close_controller_required ? "yes" : "no"} · status ${escapeHtml(financeProductionClose.latest_close_controller_status || "none")} · closed ${financeProductionClose.latest_close_controller_closed ? "yes" : "no"} · fresh ${financeProductionClose.close_controller_evidence_fresh ? "yes" : "no"} · age ${financeProductionClose.latest_close_controller_age_hours ?? "none"}h</dd>
              <dt>Accounting reconciliation</dt>
              <dd>required ${financeProductionClose.reconciliation_controller_required ? "yes" : "no"} · controller ${financeProductionClose.reconciliation_controller_configured ? "configured" : "missing"} · status ${escapeHtml(financeProductionClose.latest_reconciliation_status || "none")} · reconciled ${financeProductionClose.latest_reconciliation_reconciled ? "yes" : "no"} · fresh ${financeProductionClose.reconciliation_evidence_fresh ? "yes" : "no"} · age ${financeProductionClose.latest_reconciliation_age_hours ?? "none"}h</dd>
...
function renderAgentReleaseAutomationRuns(runs) {
  ...
  return `
    <div class="nested-item">
      ...
      <div class="muted">Orchestration controller: required ${productionOrchestration.controller_required ? "yes" : "no"} · configured ${productionOrchestration.controller_configured ? "yes" : "no"} · latest ${escapeHtml(productionOrchestration.latest_controller_status || "none")} · validated ${productionOrchestration.latest_controller_validated ? "yes" : "no"} · fresh ${productionOrchestration.controller_evidence_fresh ? "yes" : "no"} · age ${productionOrchestration.latest_controller_age_hours ?? "n/a"}h</div>
      ...
      ${
        deploymentValidation
          ? `<div class="muted">Latest manual validation: ${escapeHtml(deploymentValidation.status)} · releases ${formatInteger(deploymentValidation.release_count || 0)} · pending ${formatInteger(deploymentValidation.pending_count || 0)} · controller ${escapeHtml(deploymentValidation.controller_execution?.status || "skipped")} · checked ${escapeHtml(deploymentValidation.checked_at)}</div>`
          : ""
      }
      ${
        recentRuns.length
