...
fn RunsTasksSummary(props: &RunsTasksDataProps) -> Html {
    ...
    html! {
        <section class="runs-summary">
            <FlowMeter label="Runs" value={data.workflow_runs.data.len()} max={data.workflow_runs.data.len().max(1)} tone="neutral" />
            <FlowMeter label="Active" value={active_runs} max={data.workflow_runs.data.len().max(1)} tone="info" />
            <FlowMeter label="Failed" value={failed_runs} max={data.workflow_runs.data.len().max(1)} tone={if failed_runs > 0 { "bad" } else { "good" }} />
            <FlowMeter label="Blocked tasks" value={blocked_tasks} max={data.task_board.data.items.len().max(1)} tone={if blocked_tasks > 0 { "warn" } else { "good" }} />
            <FlowMeter label="Approvals" value={pending_approvals} max={data.approvals.data.len().max(1)} tone={if pending_approvals > 0 { "warn" } else { "good" }} />
        </section>
    }
}
...
fn RunsPanel(props: &RunsTasksDataProps) -> Html {
    html! {
        <div class="page-grid">
            ...
            <Panel title="运行记录">
                <Rows empty="没有工作流运行记录。" rows={props.data.workflow_runs.data.iter().take(12).map(|run| {
            ...
            <Panel title="证据入口">
                <KeyMetrics values={vec![
                    ("运行步骤".to_string(), "/api/workflow-runs/{id}/steps".to_string()),
                    ("状态转换".to_string(), "/api/workflow-runs/{id}/transitions".to_string()),
                    ("任务授权".to_string(), "/api/workflow-runs/{id}/task-grants".to_string()),
                    ("运行图".to_string(), "/api/workflow-runs/{id}/graph".to_string()),
                ]} />
            </Panel>
        </div>
    }
...
fn TaskBoardPanel(props: &RunsTasksDataProps) -> Html {
    ...
    html! {
        <div class="page-stack">
                        ...
                        <section class="board-column">
                            ...
                            { for filtered.into_iter().map(|item| html! {
                                <article class="board-card" key={item.id.clone()}>
                                    <strong>{ label_or(&item.title, item.work_item.as_ref().map(|work| work.title.as_str()).unwrap_or("Untitled work")) }</strong>
            ...
            <div class="page-grid">
                <Panel title="工作项">
                    <Rows empty="没有工作项。" rows={props.data.work_items.data.iter().take(8).map(|item| {
...
fn WorkflowTemplatesPanel(props: &RunsTasksDataProps) -> Html {
    html! {
        <div class="page-grid">
            <Panel title="流程模板">
                <Rows empty="没有流程模板。" rows={props.data.workflow_definitions.data.iter().take(12).map(|definition| {
...
fn DynamicPlansPanel(props: &DynamicPlansPanelProps) -> Html {
    html! {
        <div class="page-grid">
            ...
            <Panel title="动态计划">
                <Rows empty="没有动态计划。" rows={props.data.dynamic_workflow_plans.data.iter().take(12).map(|plan| {
...
fn ApprovalsPanel(props: &RunsTasksDataProps) -> Html {
    html! {
        <div class="page-grid">
            <Panel title="审批队列">
                <Rows empty="没有待处理审批。" rows={props.data.approvals.data.iter().take(12).map(|approval| {
                    (approval.status.clone(), label_or(&approval.kind, "approval").to_string(), label_or(&approval.reason, "reason").to_string())
                }).collect::<Vec<_>>()} />
            </Panel>
            <Panel title="人工确认原则">
                <KeyMetrics values={vec![
                    ("高风险动作".to_string(), "必须人工确认".to_string()),
                    ("业务写入".to_string(), "先 draft，后 approval".to_string()),
                    ("审计".to_string(), "保留 proposal、decision、actor、reason".to_string()),
                ]} />
            </Panel>
        </div>
    }
