use crate::api::{
    ...
    OntologyOnboardingToolSpec, OntologyRelease, OntologyReviewGraph, OntologyReviewGraphEdge,
...
use crate::views::semantic_i18n::{
    SemanticLang, localized_edge_type, localized_evidence_key, localized_node_type,
    localized_proposal_type, localized_risk, localized_source_mode, localized_status,
};
...
pub(crate) struct SemanticProps {
    ...
    pub(crate) ontology_release_version: String,
    ...
    pub(crate) on_create_release_candidate: Callback<MouseEvent>,
...
pub(crate) fn SemanticView(props: &SemanticProps) -> Html {
    ...
    html! {
        <div class="semantic-workbench">
            ...
            <OntologyReleaseControl
                lang={current_lang}
                run={props.onboarding_run.clone()}
                releases={props.data.ontology_releases.data.clone()}
                readiness={props.data.ontology_engine_readiness.data.clone()}
                release_version={props.ontology_release_version.clone()}
                on_release_version={props.on_release_version.clone()}
                on_create_candidate={props.on_create_release_candidate.clone()}
...
#[derive(Properties, Clone, PartialEq)]
struct OntologyReleaseControlProps {
    lang: SemanticLang,
    run: Option<OntologyOnboardingRun>,
    releases: Vec<OntologyRelease>,
    readiness: Value,
    release_version: String,
    on_release_version: Callback<InputEvent>,
    on_create_candidate: Callback<MouseEvent>,
...
#[component]
fn OntologyReleaseControl(props: &OntologyReleaseControlProps) -> Html {
