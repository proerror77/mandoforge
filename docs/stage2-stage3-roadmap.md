...
- `/api/work-items/:id/assignments` routes WorkItems to users, agents, squads,
  or teams.
- WorkItemAssignment now has explicit capability discovery with activity and
  audit evidence for routing Collaboration Layer ownership without starting
  runtime execution.
...
- The Environment entry exposes existing placement and binding records for
  ...
  evidence.
- The SessionEvent entry exposes existing event append, list, stream,
  reconnect-cursor replay, session-loop enqueue, and telemetry export surfaces
