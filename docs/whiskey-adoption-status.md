...
- Previous manual semantic synthesis / console smoke: API container ran image
  `whiskey-20260525-62609f5`; static UI serves `index-UxXs9wxD.js` and
  `index-Bbll30WL.css`; authenticated
  `POST /api/sessions/:id/semantic-synthesis-runs` returned `status=created`,
  `artifact_type=semantic_reflection_report`, `candidate_count=1`, and
  `candidate_type=session_reflection`.
- Latest scheduled semantic synthesis / scheduler smoke: API container runs
  image `whiskey-20260525-2aa1064`; static UI serves `index-BQFbRjbG.js` and
  `index-Bbll30WL.css`; authenticated `/api/scheduler/due-plan` and
  `/api/scheduler/summary` include the memory-area
