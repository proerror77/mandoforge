...
- Latest remote archive: `/opt/mandoforge-adoption/archives/mandoforge-whiskey-pilot-20260517T235226Z.tar.gz`.
- Latest local archive copy: `.mandoforge/remote-adoption/whiskey/mandoforge-whiskey-pilot-20260517T235226Z.tar.gz`.
- Latest Stage 2 strict archive copy: `.mandoforge/remote-adoption/whiskey/stage2-production-whiskey-20260517T235226Z.tar.gz`.
...
- Current Whiskey production-like pilot blocker: `Remote Computer cluster/state`.
...
| Remote Computer cluster/state | Inventory and sidecar recovery audit collected; production state sync blocked | Latest strict archive `stage2-production-whiskey-20260517T235226Z.tar.gz` reports `remote_computer_status=critical`, `production_state_sync_status=blocked`, `production_blocked_count=1`, `sidecar_recovery_evidence_status=captured`, `sidecar_recovery_run_status=noop`, and strict validation coverage with `validation_missing_endpoint_count=0`. The same archive now includes both `remote-computer-k3s/remote-computer-k3s-host-inventory-latest.txt` and `remote-computer-k3s/remote-computer-k3s-constrained-pilot-20260517T235020Z.txt`. Those record `status=preinstall_inventory`, `preflight_status=constrained_pilot_only`, `verify_status=not_installed`, `mem_available_mib=1542`, and `swap_used_mib=1428`. | Approve the constrained single-node k3s pilot and run `scripts/whiskey-remote-computer-k3s-constrained-pilot.sh --apply-host-prereqs --install-k3s`; once verify reports `ready`, run `scripts/whiskey-remote-computer-k3s-cluster-stage.sh --apply-manifests --run-evidence`. |
...
| MCP connector rollout | Passed for Whiskey authenticated GitHub repo-contents connector target | Strict evidence archive `stage2-production-whiskey-20260517T235226Z.tar.gz` still reports the `whiskey-docs` connector deployment, due rollout, and rollback path as healthy, with `validation_missing_endpoint_count=0` for the full Stage 2 coverage. The paired full pilot archive `mandoforge-whiskey-pilot-20260517T235226Z.tar.gz` shows the same bound `whiskey-docs` connector serving `github-repo-contents-authenticated` results from the private `proerror77/Goodchance@main` repository, while the strict archive now also carries both `workflow-packs/whiskey-mcp-lark-docs-scope-latest.txt` and `workflow-packs/whiskey-mcp-lark-docs-login-prompt-latest.txt` documenting that the next Lark docs/wiki step is blocked only by missing `search:docs:read` and already has a captured device-flow prompt. | Keep this as a real internal repository knowledge target; the next step is a broader Lark docs/wiki search scope or another enterprise knowledge system, not a return to private-chat evidence. |
...
After k3s is actually installed and `scripts/whiskey-remote-computer-k3s-verify.sh` reports `ready`, the repo-native follow-up command is `scripts/whiskey-remote-computer-k3s-cluster-stage.sh`. It syncs the `deploy/` bundle to Whiskey, verifies local and remote `kubectl kustomize` render counts, and only applies manifests or reruns evidence when `--apply-manifests` or `--run-evidence` are passed explicitly.
