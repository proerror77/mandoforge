...
Snapshot date: 2026-05-18 after constrained `k3s` pilot.
...
- Latest remote archive: `/opt/mandoforge-adoption/archives/mandoforge-whiskey-pilot-20260518T015410Z.tar.gz`.
- Latest local archive copy: `.mandoforge/remote-adoption/whiskey/mandoforge-whiskey-pilot-20260518T015410Z.tar.gz`.
- Latest Stage 2 strict archive copy: `.mandoforge/remote-adoption/whiskey/stage2-production-whiskey-20260518T015410Z.tar.gz`.
...
- Current Whiskey production-like pilot blocker: `Remote Computer state provider / lock-aware sync / live runner`.
...
| Remote Computer cluster/state | `k3s` pilot installed, manifests applied, and evidence refreshed; production state sync still blocked | Latest strict archive `stage2-production-whiskey-20260518T015410Z.tar.gz` still reports `stage2_status=ready`, `completion_blocked=false`, and `validation_missing_endpoint_count=0`. The paired full pilot archive `mandoforge-whiskey-pilot-20260518T015410Z.tar.gz` reports `remote_computer_status=critical`, `production_state_sync_status=blocked`, `runner_status=reserved`, `production_blocked_count=1`, and `sidecar_recovery_run_status=noop`. The strict archive now also includes `remote-computer-k3s/remote-computer-k3s-host-inventory-latest.txt` with `status=k3s_ready`, `verify_status=ready`, `br_netfilter_loaded=true`, `bridge_nf_call_iptables=1`, plus `remote-computer-k3s/remote-computer-k3s-cluster-stage-remote-render-20260518T014749Z.txt` proving the post-install cluster stage ran against Whiskey. | Remaining work is no longer k3s install. Configure a real distributed state provider (`juicefs`, `cephfs`, `longhorn-rwx`, or equivalent), add lock-aware state sync, and enable live runner mutation only after those gates are production-hardened. |
