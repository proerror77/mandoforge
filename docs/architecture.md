...
Stage 2 now includes a readiness and lease-state Remote Computer control plane: Pod template, PVC/RWX state mount placeholder, mounted state-contract ConfigMap, JuiceFS example manifest, warm-pool example manifest, KEDA ScaledObject example, readiness API, UI readiness panel, lease store, session attachment state, manual and automatic execution-job-to-lease handoff assignment, automatic warm-pool lease claiming, worker acknowledgement, assignment lifecycle status updates, reserved Pod exec transport planning, Kubernetes Pod exec intent dry-run, gated runner live-exec WebSocket capture with timeout/output bounds, env-gated assigned-Pod `file.write`, `shell.exec`, and `codex.exec`, execution-job cancellation through assigned Pod deletion, push-based Remote Computer artifact sync, scheduler-backed stale reclaim, event names, reserved runner readiness/dry-run/mutation evidence, and honest blockers for distributed filesystem, queue-depth autoscaling, automatic artifact discovery, and full job-bound Pod execution transport. Stage 3 should make this the primary sandbox substrate by creating Kubernetes Pods per session, syncing artifacts/events back to MandoForge, assigning warm pools, and integrating KEDA/HPA plus a real distributed state filesystem option such as JuiceFS CSI.
