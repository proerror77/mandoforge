...
Stage 2 now includes a readiness and lease-state Remote Computer control plane: Pod template, PVC/RWX state mount placeholder, mounted state-contract ConfigMap, JuiceFS example manifest, warm-pool example manifest, KEDA ScaledObject example, readiness API, UI readiness panel, lease store, session attachment state, manual and automatic execution-job-to-lease handoff assignment, worker acknowledgement, assignment lifecycle status updates, reserved Pod exec transport planning, Kubernetes Pod exec intent dry-run, gated runner live-exec WebSocket capture, env-gated assigned-Pod `shell.exec`, scheduler-backed stale reclaim, event names, reserved runner readiness/dry-run/mutation evidence, and honest blockers for production warm-pool assignment, distributed filesystem, queue-depth autoscaling, artifact sync, timeout/cancel, and full job-bound Pod execution transport. Stage 3 should make this the primary sandbox substrate by creating Kubernetes Pods per session, syncing artifacts/events back to MandoForge, assigning warm pools, and integrating KEDA/HPA plus a real distributed state filesystem option such as JuiceFS CSI.
