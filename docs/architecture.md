...
- Usage groundwork includes `GET /api/usage`, which aggregates provider request/response counts, prompt/completion/total tokens from provider responses, configured per-request and per-1K-token provider pricing, tool status counts, tool runtime, approval counts, sessions, and session events for the current tenant. Admins can persist current usage/cost snapshots through `POST /api/usage/rollups` and review them in the static console.
...
- Provider Settings in the static console can create/update stored providers with daily request budgets plus per-request and per-1K-token pricing config.
