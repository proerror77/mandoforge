...
- Approval v2 supports delegated approver enforcement, expiry, argument modification, and env-gated webhook notification delivery for pending approvals through `MANDOFORGE_APPROVAL_WEBHOOK_URL`.
...
- Usage/cost tracking includes warning/critical provider budget alerts, an Admin-only alert listing endpoint, audited acknowledgement route, and a webhook delivery route controlled by `MANDOFORGE_COST_ALERT_WEBHOOK_URL`; the static Usage panel can trigger delivery or acknowledgement and show the latest status.
