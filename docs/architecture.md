...
- Usage groundwork includes `GET /api/usage`, which aggregates provider request/response counts, prompt/completion/total tokens from provider responses, configured per-request and per-1K-token provider pricing, tool status counts, tool runtime, approval counts, sessions, and session events for the current tenant.
