...
- Policy Center supports active YAML policy inspection, single-tool simulation, batch policy testing, validated persisted policy revisions, revision diffing against the active runtime policy, fixed safety rollout gates, activation metadata, and audit logging for policy simulation/test/revision actions; runtime hot-swap, configurable test suites, and staged rollout percentages remain reserved.
...
- Codex App Server groundwork includes an env-gated `CodexAppServerClient` boundary and Admin-only adapter routes for health, thread creation, turn creation, command execution, interrupt, and artifact sync into MandoForge session artifacts. If `MANDOFORGE_CODEX_APP_SERVER_URL` is unset, the adapter fails closed and `codex.exec` continues using the CLI path. The static Admin Console now exposes these steering actions, imports returned artifacts into the timeline/audit path, and renders reserved/fail-closed responses clearly; fallback orchestration is still later-stage work.
