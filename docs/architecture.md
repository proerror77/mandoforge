...
The current repo does not yet implement this substrate as an execution path. Today it has K8s API/worker/scheduler skeletons, queue-backed execution jobs, worker leases, Docker shell sandbox support, a worker readiness gate, and a Remote Computer readiness skeleton with a Pod template, restricted service account, RWX PVC placeholder, and deny-by-default NetworkPolicy. It does not yet create a Pod per session, maintain a warm pool, or mount a real distributed Memory/Notes/Skills filesystem into leased agent Pods.
...
Stage 2 now includes a readiness-only Remote Computer control plane: Pod template, PVC/RWX state mount placeholder, readiness API, UI readiness panel, event names, and honest blockers for warm pool, distributed filesystem, and queue-depth autoscaling. Stage 3 should make this the primary sandbox substrate by leasing Pods per session, syncing artifacts/events back to MandoForge, adding warm pools, and integrating KEDA/HPA plus a distributed state filesystem option such as JuiceFS CSI.
