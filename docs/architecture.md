...
- Codex App Server groundwork includes an env-gated `CodexAppServerClient` boundary and Admin-only adapter routes for health, thread creation, turn creation, command execution, and interrupt. If `MANDOFORGE_CODEX_APP_SERVER_URL` is unset, the adapter fails closed and `codex.exec` continues using the CLI path. The static Admin Console now exposes these steering actions and renders reserved/fail-closed responses clearly; artifact sync and fallback orchestration are still later-stage work.
