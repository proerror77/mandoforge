...
- Provider governance groundwork includes Admin-only provider registry routes, `provider_access` rows per team, model allowlist enforcement when creating a team-scoped agent, runtime provider selection from active stored provider rows, and daily request/cost budget gates before `llm.request` events are emitted.
