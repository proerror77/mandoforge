...
- Policy Center supports active YAML policy inspection, single-tool simulation, batch policy testing, validated persisted policy revisions, activation metadata, and audit logging for policy simulation/test/revision actions; runtime hot-swap, diffing, and staged rollout remain reserved.
