...
- Codex App Server groundwork includes an env-gated `CodexAppServerClient` boundary and Admin-only adapter routes for health, thread creation, turn creation, command execution, interrupt, and artifact sync into MandoForge session artifacts. If `MANDOFORGE_CODEX_APP_SERVER_URL` is unset, the adapter fails closed. Approved `codex.exec` can run with `auto`, `cli`, or `app-server` execution strategy; `auto` tries App Server when configured and records `codex.task.fallback` before returning to the CLI path if the App Server attempt fails. The static Admin Console exposes steering actions, imports returned artifacts into the timeline/audit path, and renders reserved/fail-closed responses clearly.
