...
- Managed CLI runtimes are transitioning from plain subprocess tools to runtime adapters. The current compatibility facade is `agent_cli.exec`: it still produces legacy tool results, but managed `codex_cli` and `claude_code` profiles parse JSONL / stream-json output into canonical `runtime_adapter.event` session events with basic secret-key redaction and event-count limits. Managed `codex_cli` runs also derive structured runtime turn records from the parsed JSONL: `runtime.turn.started`, `runtime.item`, `runtime.tool_call`, `runtime.usage`, `runtime.final`, and `runtime.turn.completed` capture resume handles, `--output-schema` metadata, schema validation, usage/timing, collected items, and final-message artifacts. `Environment.runtime_profile_id` is now the canonical adapter binding for managed sessions; requested CLI profiles must match that environment binding before falling back to handoff or agent runtime profiles, and the legacy env-var allowlist is only a no-binding fallback. The next boundary correction is to route Claude Code CLI and Codex App Server through the same normalized turn taxonomy.
...
- Worker queue polling honors Environment placement. Workers can send `x-mandoforge-environment-id` (or run `mandoforge-worker` with `WORKER_ENVIRONMENT_ID`) to list and claim only session-loop and execution jobs whose session is bound to that Environment; mismatched direct run attempts are treated as not claimable.
- Session event streaming has reconnect cursors: `GET /api/sessions/:id/stream?after_seq=N` and SSE `Last-Event-ID` replay only events with a higher session event sequence, and emitted SSE events use the session event `seq` as their `id` so clients can resume without losing progress after a disconnect.
