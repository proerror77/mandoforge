...
- External worker mode is API-drained for memory/Postgres/Redis/NATS queues. Redis Streams enqueue and drain are available through `MANDOFORGE_EXECUTION_QUEUE_BACKEND=redis`; Core NATS publish and queue-subscription drain are available through `MANDOFORGE_EXECUTION_QUEUE_BACKEND=nats`. `MANDOFORGE_EXECUTION_QUEUE_BACKEND=nats_jetstream` or `jetstream` exposes only the reserved JetStream command/config boundary and returns fail-closed errors for real queue operations.
...
- Keep `execution_queue_broker.rs` as the broker backend boundary. Redis Stream enqueue/group/read/ack operations and Core NATS publish/queue-subscription drain operations are implemented behind the `ExecutionQueueBackend` facade and locally verified with mock TCP tests. NATS mode is not a JetStream durability claim. JetStream stream/consumer/publish command builders exist for the next implementation slice, but the backend fails closed until real publish, durable pull-consumer drain, explicit ack, and redelivery handling are implemented and tested.
...
- Keep `GET /api/execution-jobs/worker-readiness` as the operator gate for this boundary. It reports active queue semantics, worker mode, queued/running/retryable/failed pressure, stale leases, K8s worker manifest coverage, HPA/KEDA autoscaling manifest presence, parsed scale targets/min/max replicas, attention items, and runbook actions. When the reserved JetStream backend is selected, readiness returns a critical attention item instead of treating fail-closed queue operations as production readiness. This exposes the current gaps without claiming JetStream durability, validated queue-depth autoscaling, or hard-isolated worker pools.
...
The current repo does not yet implement this substrate as a Pod execution path. Today it has K8s API/worker/scheduler skeletons, queue-backed execution jobs, worker leases, Docker shell sandbox support, a worker readiness gate, and a Remote Computer control-plane skeleton with a Pod template, restricted service account, RWX PVC placeholder, JuiceFS CSI example manifest, warm-pool Deployment example, KEDA ScaledObject example, deny-by-default NetworkPolicy, `remote_computers` / `remote_computer_leases` storage, lease heartbeat/release/fail APIs, `remote_computer_session_attachments`, stale attachment listing, Admin-only stale reclaim, manual execution-job-to-lease assignment, automatic worker assignment to an active matching Remote Computer lease during normal execution-job drains, assignment lifecycle status updates for completed/retry-released/failed jobs, a reserved `RemoteComputerRunner` boundary, a Kubernetes runner with dry-run/probe evidence, Pod exec intent dry-run, and doubly gated Pod create/delete HTTP calls, Admin-only runner readiness/dry-run/mutate routes, and session event/audit records. It does not yet execute approved tools inside Kubernetes Pods, attach to a real Pod exec transport, assign sessions to a warm pool, or mount a real distributed Memory/Notes/Skills filesystem into leased agent Pods.
...
Stage 2 now includes a readiness and lease-state Remote Computer control plane: Pod template, PVC/RWX state mount placeholder, JuiceFS example manifest, warm-pool example manifest, KEDA ScaledObject example, readiness API, UI readiness panel, lease store, session attachment state, manual and automatic execution-job-to-lease handoff assignment, worker acknowledgement, assignment lifecycle status updates, reserved Pod exec transport planning, Kubernetes Pod exec intent dry-run, scheduler-backed stale reclaim, event names, reserved runner readiness/dry-run/mutation evidence, and honest blockers for production warm-pool assignment, distributed filesystem, queue-depth autoscaling, and Pod execution transport. Stage 3 should make this the primary sandbox substrate by creating Kubernetes Pods per session, syncing artifacts/events back to MandoForge, assigning warm pools, and integrating KEDA/HPA plus a real distributed state filesystem option such as JuiceFS CSI.
