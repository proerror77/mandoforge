...
- External worker mode is API-drained for memory/Postgres/Redis/NATS queues. Redis Streams enqueue and drain are available through `MANDOFORGE_EXECUTION_QUEUE_BACKEND=redis`; Core NATS publish and queue-subscription drain are available through `MANDOFORGE_EXECUTION_QUEUE_BACKEND=nats`.
...
- Keep `GET /api/execution-jobs/worker-readiness` as the operator gate for this boundary. It reports active queue semantics, worker mode, queued/running/retryable/failed pressure, stale leases, K8s worker manifest coverage, HPA/KEDA autoscaling manifest presence, parsed scale targets/min/max replicas, attention items, and runbook actions. This exposes the current gaps without claiming JetStream durability, validated queue-depth autoscaling, or hard-isolated worker pools.
