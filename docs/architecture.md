...
- Provider governance groundwork includes Admin-only provider registry routes, `provider_access` rows per team, model allowlist enforcement when creating a team-scoped agent, runtime provider selection from active stored provider rows, and a daily request budget gate before `llm.request` events are emitted.
- Usage groundwork includes `GET /api/usage`, which aggregates provider request/response counts, configured per-request provider pricing, tool status counts, tool runtime, approval counts, sessions, and session events for the current tenant.
...
- Provider Settings in the static console can create/update stored providers with daily request budgets and per-request pricing config.
- Evaluation groundwork includes eval datasets, cases, and version-bound run records with deterministic Stage 2 graders for policy decisions, tool allowlist coverage, SQL safety, sandbox path checks, and final-answer required fragments.
...
- MCP Gateway execution is now available through `mcp.call` when configured; global server allowlists are enforced by the gateway config, and team-scoped sessions must also pass the persisted MCP server registry/tool allowlist before the HTTP call.
