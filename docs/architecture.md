...
- Keep `GET /api/execution-jobs/worker-readiness` as the operator gate for this boundary. It reports active queue semantics, worker mode, queued/running/retryable/failed pressure, stale leases, K8s worker manifest coverage, missing autoscaling manifest, attention items, and runbook actions. This exposes the current gaps without claiming JetStream durability, autoscaling, or hard-isolated worker pools.
