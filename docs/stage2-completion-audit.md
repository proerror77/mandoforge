...
| --- | --- | --- |
| Multi-tenancy with org/team/project scopes | `organizations`, `teams`, `projects`, `memberships`, and `tenant_invitations` exist; incoming `x-mandoforge-tenant-id` must match the runtime tenant and fails closed otherwise; scoped agent/session access is tested by `admin_can_manage_stage2_governance_scope`, `tenant_provisioning_bootstrap_creates_owner_scope_and_audit`, `tenant_invitations_create_accept_revoke_and_audit_membership`, and `request_tenant_header_must_match_runtime_tenant`. | Partial: runtime still uses one default tenant boundary, so production cross-tenant isolation is not complete. |
...
| Worker queue, leases, retries, and resume | In-memory/Postgres execution queue, Rust `mandoforge-worker` binary, shell worker loop, K8s worker Deployment, worker HPA skeleton, worker KEDA queue-depth ScaledObject, Redis Streams backend, Core NATS backend, NATS JetStream durable handoff, lease/retry behavior, stale running reclaim, queue-backed Codex App Server execution, Remote Computer manual and automatic worker lease assignment, assignment lifecycle updates, and `GET /api/execution-jobs/worker-readiness` are implemented. The readiness gate reports queue semantics, worker mode, queued/running/retryable/failed pressure, stale leases, K8s worker manifest coverage, HPA/KEDA autoscaling manifest presence, parsed scale targets/min/max replicas, trigger types, queue-depth scaling evidence, validation status, and runbook actions; the static Worker Dashboard renders it. Tests include `execution_queue_tracks_job_lifecycle`, `queue_backed_worker_defers_approved_tool_until_job_run`, `queue_backed_worker_auto_assigns_active_remote_computer_lease`, Redis broker tests, Core NATS broker publish/message parsing tests, and JetStream command/publish/drain/ack tests. `cargo check -p mandoforge-api --bin mandoforge-worker` passes. | Partial: JetStream durable handoff and queue-depth KEDA manifests are now implemented, but production worker hardening and load-validated autoscaling remain incomplete and API/UI-visible. |
| Agent Remote Computer substrate | K8s readiness skeleton exists for a Pod-based remote computer direction: `deploy/k8s/agent-remote-computer.yaml`, service account, RWX state PVC placeholder, JuiceFS CSI example manifest, warm-pool Deployment example, KEDA ScaledObject example, opt-in `deploy/kustomization.yaml` pilot bundle, deny-by-default NetworkPolicy, `GET /api/remote-computers/readiness`, static UI readiness panel, `remote_computers` / `remote_computer_leases` storage, lease heartbeat/release/fail APIs, `remote_computer_session_attachments`, `remote_computer_job_assignments`, manual and automatic worker assignment to active matching Remote Computer leases, assignment completed/released/failed lifecycle events, worker acknowledgement for assigned jobs, reserved Pod exec transport planning, Pod exec dry-run intent, stale attachment listing, Admin-only stale reclaim, scheduler due-plan/run integration for stale reclaim, a reserved `RemoteComputerRunner` boundary, explicit `KubernetesRemoteComputerRunner` skeleton, Admin-only `GET /api/remote-computers/runner/readiness`, Admin-only audited `POST /api/remote-computers/runner/dry-run`, Admin-only audited `POST /api/remote-computers/runner/mutate`, gated Kubernetes Pod create/delete HTTP calls, UI runner/attachment/handoff evidence, and `remote_computer.*` session event/audit records. | Partial: normal worker drains can now create and finalize handoff assignments from existing leases, and the runner can produce audited Pod exec intent dry-runs, but Pod mutation is explicit and doubly gated; actual tool execution still uses the existing approved worker path rather than Pod exec; JuiceFS, warm pool, and KEDA are bundled as opt-in pilot manifests but not production-hardened, there is no real distributed Memory/Notes/Skills filesystem, and no tool execution inside leased Pods yet. |
...
```text
cargo test -p mandoforge-api: 142 passed
...
5. Production worker hardening and load-validated autoscaling beyond the existing Rust worker binary, K8s worker skeleton, HPA/KEDA manifests, and broker-backed Redis/Core NATS/JetStream handoff.
