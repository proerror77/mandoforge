...
- `stage2_production`: production-readiness evidence lanes such as provider
  rollout, MCP rollout, approval notifications, eval/release, observability,
  managed workflow runtime proof, and UI polish. These are enforced by the
  Stage 2 production gate, but they are not Agent OS core gaps.
