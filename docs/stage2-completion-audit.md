...
| Worker queue, leases, retries, and resume | In-memory/Postgres execution queue, Rust `mandoforge-worker` binary, shell worker loop, K8s worker Deployment, worker HPA skeleton, Redis Streams backend, Core NATS backend, reserved NATS JetStream configuration/command boundary, lease/retry behavior, stale running reclaim, queue-backed Codex App Server execution, Remote Computer manual and automatic worker lease assignment, assignment lifecycle updates, and `GET /api/execution-jobs/worker-readiness` are implemented. The readiness gate reports queue semantics, worker mode, queued/running/retryable/failed pressure, stale leases, K8s worker manifest coverage, HPA/KEDA autoscaling manifest presence, parsed scale targets/min/max replicas, skeleton validation status, and runbook actions; the static Worker Dashboard renders it. Tests include `execution_queue_tracks_job_lifecycle`, `queue_backed_worker_defers_approved_tool_until_job_run`, `queue_backed_worker_auto_assigns_active_remote_computer_lease`, Redis broker tests, Core NATS broker publish/message parsing tests, and JetStream command/fail-closed tests. `cargo check -p mandoforge-api --bin mandoforge-worker` passes. | Partial: Core NATS remains non-durable, the JetStream backend is selectable only as a reserved fail-closed boundary without real publish/drain/ack, and production worker hardening/validated autoscaling remains incomplete but is now API/UI-visible. |
...
```text
cargo test -p mandoforge-api: 134 passed
...
5. Production JetStream-style queue durability and worker hardening/validated autoscaling beyond the existing Rust worker binary, K8s worker skeleton, HPA skeleton, and reserved JetStream command/config boundary.
