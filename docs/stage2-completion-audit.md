...
| Agent Remote Computer substrate | K8s readiness skeleton exists for a Pod-based remote computer direction: `deploy/k8s/agent-remote-computer.yaml`, service account, RWX state PVC placeholder, JuiceFS CSI example manifest, warm-pool Deployment example, KEDA ScaledObject example, deny-by-default NetworkPolicy, `GET /api/remote-computers/readiness`, static UI readiness panel, `remote_computers` / `remote_computer_leases` storage, lease heartbeat/release/fail APIs, `remote_computer_session_attachments`, `remote_computer_job_assignments`, worker acknowledgement for assigned jobs, reserved Pod exec transport planning, stale attachment listing, Admin-only stale reclaim, scheduler due-plan/run integration for stale reclaim, a reserved `RemoteComputerRunner` boundary, explicit `KubernetesRemoteComputerRunner` skeleton, Admin-only `GET /api/remote-computers/runner/readiness`, Admin-only audited `POST /api/remote-computers/runner/dry-run`, Admin-only audited `POST /api/remote-computers/runner/mutate`, gated Kubernetes Pod create/delete HTTP calls, UI runner/attachment/handoff evidence, and `remote_computer.*` session event/audit records. | Partial: Pod mutation is explicit and doubly gated, not used by normal session scheduling; attachment/reclaim/handoff/transport planning is control-plane state only; JuiceFS, warm pool, and KEDA are example-only, there is no real distributed Memory/Notes/Skills filesystem, and no tool execution inside leased Pods yet. |
