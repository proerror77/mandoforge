...
| Provider governance | OpenAI-compatible provider transport exists; API keys can be direct env values or Vault references. Admin provider registry APIs are implemented, team-level `provider_access` rows and model allowlist enforcement are implemented for team-scoped agent creation, and session runs now resolve active stored provider rows before falling back to env/mock. Stored provider configs can enforce daily request and cost budgets. The static Provider Settings form can create/update stored providers with budget plus per-request and per-1K-token pricing config. | Richer provider status management is not implemented. |
