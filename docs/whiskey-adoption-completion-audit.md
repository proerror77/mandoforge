...
| Publish a pullable GHCR image. | Deploy workflow `25991927137` succeeded for `ghcr.io/proerror77/mandoforge/mandoforge-api:09aa783`; Whiskey `docker compose ps` shows API and worker running that image. | Passed. |
...
| Archive every adoption evidence run remotely. | Latest full remote archive is `/opt/mandoforge-adoption/archives/mandoforge-whiskey-pilot-20260517T132402Z.tar.gz`; latest strict archive is `/opt/mandoforge-adoption/archives/stage2-production-whiskey-20260517T132402Z.tar.gz`. | Passed. |
| Sync evidence locally. | Latest local copies are `.mandoforge/remote-adoption/whiskey/mandoforge-whiskey-pilot-20260517T132402Z.tar.gz` and `.mandoforge/remote-adoption/whiskey/stage2-production-whiskey-20260517T132402Z.tar.gz`. | Passed. |
| Verify Stage 2 strict archive. | `ALLOW_BLOCKED=1 ./scripts/verify-stage2-evidence-archive.sh .mandoforge/remote-adoption/whiskey/stage2-production-whiskey-20260517T132402Z.tar.gz` passes. Archive summary reports `stage2_status=ready`, `completion_blocked=false`, `open_gap_count=0`, `validation_missing_endpoint_count=0`, and `validation_stale_endpoint_count=0`. | Passed for Whiskey pilot evidence coverage. |
...
The Whiskey production-like pilot is repeatable, archived, locally synced, GHCR-backed, and broadly evidenced. Tenant routing/RLS plus WorkflowPack immutable version updates, persisted onboarding profile assets, and onboarding assessment now pass for the Whiskey tenant-routed pilot. The objective should not be marked fully complete because the requested adoption plan includes Worker / Remote Computer cluster-state completion, and Remote Computer still has explicit production blockers.
...
- continue Stage 3 customer pack install defaults and external connector-data-quality hardening while keeping the Remote Computer and external enterprise target blockers visible.
