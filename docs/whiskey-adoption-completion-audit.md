...
| --- | --- | --- |
| Push local commits including `579e927 Package runtime assets in deploy image`. | `git log --all --oneline` contains `579e927 Package runtime assets in deploy image`; latest pushed ledger commit is `7d369fe Record Whiskey WorkflowPack rollback evidence`. | Passed. |
...
| Publish a pullable GHCR image. | Deploy workflow `25988435460` succeeded for `ghcr.io/proerror77/mandoforge/mandoforge-api:8e69814`; Whiskey `docker compose ps` shows API and worker running that image. | Passed. |
...
| Archive every adoption evidence run remotely. | Latest full remote archive is `/opt/mandoforge-adoption/archives/mandoforge-whiskey-pilot-20260517T104212Z.tar.gz`; latest strict archive is `/opt/mandoforge-adoption/archives/stage2-production-whiskey-20260517T104212Z.tar.gz`. | Passed. |
| Sync evidence locally. | Latest local copies are `.mandoforge/remote-adoption/whiskey/mandoforge-whiskey-pilot-20260517T104212Z.tar.gz` and `.mandoforge/remote-adoption/whiskey/stage2-production-whiskey-20260517T104212Z.tar.gz`. | Passed. |
| Verify Stage 2 strict archive. | `ALLOW_BLOCKED=1 ./scripts/verify-stage2-evidence-archive.sh .mandoforge/remote-adoption/whiskey/stage2-production-whiskey-20260517T104212Z.tar.gz` passes. Archive summary reports `stage2_status=ready`, `completion_blocked=false`, `open_gap_count=0`, `validation_missing_endpoint_count=0`, and `validation_stale_endpoint_count=0`. | Passed for Whiskey pilot evidence coverage. |
...
| Tenant routing/RLS adoption. | Tenant controller evidence is collected and fresh; readiness still reports `runtime still serves one configured tenant instead of routing per tenant`. | Not production-complete; real tenant-routed runtime is required. |
...
| Stage 3 WorkflowPack adoption on Whiskey. | Latest full archive `workflow-packs/summary.txt` reports `workflow_pack_status=archived_after_rollback`, `install_status=installed`, `stage_status=staged`, `release_status=released`, `rollback_status=rolled_back`, `archive_status=archived`, `eval_gate_status=passed`, `release_gate_status=passed`, `rolled_back_get_status=rolled_back`, `rolled_back_list_count=1`, `archived_get_status=404`, and `active_after_archive_count=0`. | Passed for Whiskey pilot lifecycle. |
...
| --- | --- | --- |
| Tenant routing/RLS production adoption | Whiskey still runs `single_runtime_tenant`; controller evidence proves the gate and RLS signals, not real tenant-routed serving. | Implement and validate real tenant-routed runtime before marking this lane production-ready. |
...
The Whiskey production-like pilot is repeatable, archived, locally synced, GHCR-backed, and broadly evidenced. The objective should not be marked fully complete because the requested adoption plan includes Worker / Remote Computer cluster-state and Tenant routing/RLS completion, and both still have explicit production blockers.

The next concrete work should be one of:

- implement real tenant-routed runtime and rerun tenant isolation evidence; or
- approve/configure a real cluster path for Remote Computer state/sidecar evidence; or
- continue Stage 3 WorkflowPack version-update/customer-onboarding hardening while keeping the two production blockers visible.
