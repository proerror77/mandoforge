...
The evidence script seeds a `whiskey-docs` connector on the Whiskey pilot team, keeps the `search` tool allowlisted, creates a due rollout when one is not already pending, enables strict MCP due-run plus rollback evidence, and routes `/v1/call` through a read-only public GitHub repository search target. This validates the real MandoForge MCP gateway HTTP boundary and rollout controller hooks while exercising a non-pilot external read target.
