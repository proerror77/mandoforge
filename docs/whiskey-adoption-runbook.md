...
The evidence script seeds a `whiskey-docs` connector on the Whiskey pilot team, keeps the `search` tool allowlisted, creates a due rollout when one is not already pending, enables strict MCP due-run plus rollback evidence, and routes `/v1/call` through an authenticated GitHub repository search target using the Whiskey host's existing `gh` credentials. This validates the real MandoForge MCP gateway HTTP boundary and rollout controller hooks while exercising a credentialed non-pilot external read target.
