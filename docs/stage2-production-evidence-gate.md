...
`MANDOFORGE_STAGE2_PRODUCTION_CLUSTER_ID`, and the all-up manifest must bind the
same worker pool through `MANDOFORGE_STAGE2_WORKER_POOL`. State-sync evidence must name the
state claim plus checked state-contract paths with matching per-path cluster id
and state-claim identity, and the claim must match
`MANDOFORGE_STAGE2_REMOTE_STATE_CLAIM`; the backend must match
`MANDOFORGE_STAGE2_REMOTE_STATE_BACKEND`; details must include passed statuses and audit id,
...
rows do not satisfy the reported counts. The controller id, policy store id,
and deployment id must match `MANDOFORGE_STAGE2_POLICY_CONTROLLER_ID`,
`MANDOFORGE_STAGE2_POLICY_STORE_ID`,
`MANDOFORGE_STAGE2_POLICY_DEPLOYMENT_ID`, and the all-up archive manifest's
policy rollout target. The matching in-cluster template
