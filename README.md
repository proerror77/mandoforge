...
Approved execution also enters an execution queue facade before it is drained. The queue is Postgres-backed when `DATABASE_URL` is set and in-memory otherwise. The default worker is `InlineExecutionWorker`; set `MANDOFORGE_EXECUTION_WORKER=queue` to enqueue approved jobs for an external worker loop, then drain a job through `POST /api/execution-jobs/:id/run`.
